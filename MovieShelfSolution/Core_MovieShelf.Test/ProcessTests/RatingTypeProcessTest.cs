...
{
    [TestFixture]
    public class RatingTypeProcessTest : TestBase
    {
        ...
        private static RatingTypeProcess GetClassUnderTest(IBaseLookupDal<Domain.RatingType, IDomainList<Domain.RatingType, IEnumerable<Domain.RatingType>>, int> provider)
        {
            return new RatingTypeProcess(provider);
        }
        
        [Test]
        public void TestGet()
        {
            var dalInstance = new MockObjects.MockHelper().GetRatingTypeDal();
        ...
        {
            var dalInstance = new MockObjects.MockHelper().GetRatingTypeDalGoodMock();
            var classUnderTest = GetClassUnderTest(dalInstance);
            var actualResult = classUnderTest.Get(1, true);
