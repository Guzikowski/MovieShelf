...
namespace Core_MovieShelf.Test.DomainTests
{
	...
	public class BaseLookupTest : AutoMockerBase<BaseLookup>
	{
		...
		{
			public ConcreteBaseLookup()
			{
			}
			public ConcreteBaseLookup(IBaseLookupContract baseDomain)
				: base(baseDomain)
			{
			}
		}
		...
		public void TestName()
		{
			...
			CheckProperty(p => p.Name, string.Empty, string.Empty, UnitTestValues.LastName1);
		}
		/// <summary>
		...
		public void TestConvertToContract()
		{
            ...
            var actualResult = CreateTargetObject().ConvertToContract((BaseLookup)testObject, new BaseLookupContract());
		...
		{
			var testObject = new BaseLookupContract();
			Assert.IsNotNull(testObject);
			...
			var actualResult = new ConcreteBaseLookup(testObject);
			Assert.IsNotNull(actualResult);
			Assert.IsTrue(actualResult.HasChanged == testObject.HasChanged);
			Assert.IsTrue(actualResult.Id == testObject.Id);
			Assert.IsTrue(actualResult.IsNew == testObject.IsNew);
		...
		public class PrivateAccessor
		{
			...
			private const string FullClassName = "Core_MovieShelf.Domain.BaseLookup";
